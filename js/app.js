...
/***FUNCIONES****/
//Crea el objeto Carta implementando un prototipo
class Carta{
  constructor(x, y, ancho, largo, info){
    this.x = x;
    this.y = y;
    this.ancho = ancho;
    this.largo = largo;
...
//dibuja cada una de las cartas dentro del tablero
const tablero = () => {
 let carta;
 let x = inicioX;
 let y = inicioY;
 ...
 //console.log(cartas_array);
}
/*intercambia las cartas de lugar*/
const barajear = () => {
  let i, j, k;
  let temporal;
  const lon = cartas_array.length;
  for(j=0; j<lon*3; j++){
...
function ajusta(xx, yy){
  const posCanvas = canvas.getBoundingClientRect();/*nos da las coordenadas absolutas del canvas, largo y alto que fue clickeado*/
  let x = xx -posCanvas.left;//nos da la posicion x y y con repecto a la posicion original del canvas
...
function volteaCarta(){
  if(iguales==false){
    cartas_array[cartaPrimera].dibujarCarta();
    cartas_array[cartaSegunda].dibujarCarta();
